...
pub fn choleskiDecomposition(comptime n: i32, A: Matrix(n, n)) Matrix(n, n) {
    var result = A;
    _ = Choleski_LU_Decomposition(&result.data, n);
    return result;
}

pub fn choleskiInverse(comptime n: i32, LU: Matrix(n, n)) Matrix(n, n) {
    var result = LU;
    _ = Choleski_LU_Inverse(&result.data, n);
    return result;
}
...
pub fn multiplyMatrices(
    ...
    A: *const AType,
    B: *const BType,
) Matrix(AType.colCount(), BType.colCount()) {
    var result = Matrix(AType.colCount(), BType.colCount()).zeroed();
    Multiply_Matrices(&result.data, &A.data, AType.rowCount(), AType.colCount(), &B.data, BType.colCount());
    return result;
}
